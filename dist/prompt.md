...
If they pasted a job description, extract must-haves, nice-to-haves, the implied archetype and level, and the company's own vocabulary using Appendix: jd-and-ats. A posting answers three of the four rows above on its own, so with a JD in hand this step should cost **zero questions**.

Read Appendix: targeting for the archetype, level and company tables. Its five target questions are a fallback for a candidate who has given you nothing to read, not a script to run every time.
