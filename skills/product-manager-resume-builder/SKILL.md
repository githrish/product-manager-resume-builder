...
description: Build, rebuild, and tailor product management resumes — from an existing draft or from nothing at all. Use when the user wants to write or improve a PM resume, strengthen product resume bullets, quantify product achievements, tailor a resume to a product job description, check ATS match for a product role, switch into product management from engineering, design, marketing, data, sales, support, ops, or consulting, or break in early-career as an APM, product analyst, product owner, or product intern.
---
...
| 6 | Tailoring | Paste a job description |
| 7 | Audit | Receive the failure list and the estimates to defend |
...
Announce each phase, and give the step number so the candidate always knows where they are. Run them in order.

### Phase 0 — Intake and diagnosis · Step 1 of 7
...
If they struggle to recall, give them the artifact prompts from `references/from-scratch.md`. Then run Phase 1.
...
### Phase 1 — Target lock · Step 2 of 7
...
If they pasted a job description, extract must-haves, nice-to-haves, the implied archetype and level, and the company's own vocabulary, using `references/jd-and-ats.md` to classify requirements properly. If they only named a title, ask the five target questions in `references/targeting.md`.
...
### Phase 2 — Evidence excavation · Step 3 of 7
...
### Phase 3 — Bullet rebuild · Step 4 of 7
...
Read `references/bullets.md` for the formula, the ownership ladder, the metric taxonomy, the Estimation Protocol, and the anti-patterns. Check every rewritten bullet against that anti-pattern list before showing it.

### Phase 4 — Master resume assembly · Step 5 of 7

**Build the master first, always.** The master is the superset — every role, every bullet worth keeping, every metric — and is **never sent to anyone**. Each application is a view of it.

Never edit an application copy directly. Edit the master, then re-derive. Tailoring a tailored copy compounds drift, and rewriting is where fabrication creeps in.

Read `references/templates.md` for the five layouts, the one-page cutting order, and the formatting spec. **Offer the template choice with a recommendation; don't pick silently.**
...
### Phase 5 — The tailoring loop · Step 6 of 7

Once per application, always derived from the master. Confirm market and company archetype first.

1. **Classify the requirements** using `references/jd-and-ats.md`, separating genuine must-haves from wish-list items and flagging real dealbreakers. Then extract keywords: hard skills, tools, methodologies, domain terms, in the posting's exact phrasing.
2. **Match score:** `(required keywords present ÷ total required) × 100`, counting only the required set. Report it with the interpretation bands in that reference.
3. **Integrate naturally** — keywords inside real sentences. Never a stuffed block, never white text.
4. **Reorder, don't rewrite.** Promote bullets matching this JD to the top of each role.
5. **Gap honesty.** For each unmet must-have: cover in the cover letter, cover with adjacent evidence, or accept it as a genuine gap. Don't paper over it.
6. **Run the parser check** from `references/jd-and-ats.md` before the file is sent. Most rejections blamed on keywords are parse failures.

File naming: `Lastname_PM_Company_YYYY-MM.pdf`. Suggest a tracking table with match score, archetype, date applied, and outcome.

```
## Tailoring Report — [Role] at [Company]
**Match score:** X% ([Y] of [Z] required keywords)
**Market conventions applied:** [country — length, personal-details rule]
**Company archetype:** [which, and the one emphasis shift it drove]
**Verdict:** [Strong fit / Worth applying / Stretch — why]
**Changes made:** [list]
**Keywords added:** [list, and where each landed]
**Still missing:** [list, and how to handle each]
```

### Phase 6 — Final audit · Step 7 of 7
...
- [ ] The Phase 1 archetype is legible in the top third of the page
...
**Master resume:** [page count, where it stands]
**Versions derived:** [list]
**Next steps:** [tailoring targets, LinkedIn alignment]
```
...
|---|---|---|
| `references/from-scratch.md` | Phase 0 Track B, Phase 2 | Blank Page Protocol, Story Inventory, recall prompts |
| `references/targeting.md` | Phase 1 | PM archetypes, level calibration, company archetypes, target questions |
| `references/markets.md` | Phase 1, Phase 4 | Country conventions, the one-page exception, text-only rule |
| `references/transitions.md` | Phase 1 onward, transitioning in | Level mapping for experienced switchers, domain leverage, the three routes, how to rebuild the old resume |
| `references/breaking-in.md` | Phase 1 onward, early career | Six entry targets, transferable evidence map, PO inversion, Title Honesty Rule, Gap-Filling Sprint |
| `references/jd-and-ats.md` | Phase 5 | Requirement classification, dealbreakers, parser failures, match-score bands |
| `references/bullets.md` | Phase 3 | Bullet formula, ownership ladder, metric taxonomy, Estimation Protocol, anti-patterns |
| `references/templates.md` | Phase 4 | Five layouts, one-page cutting order, formatting spec |
