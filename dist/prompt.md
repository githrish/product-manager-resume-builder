...
7. **Write like the candidate, not like a model.** Trailing "-ing" clauses, em dashes, stacked verbs and forced triples are what make a resume read as generated. Appendix: writing-style has the full list. Check every bullet against it before showing it.
...
If they pasted a job description, that **is** evidence of intent, not a guess about it. Extract must-haves, nice-to-haves, the implied archetype and level, and the company's own vocabulary using Appendix: jd-and-ats. A posting settles all four rows on its own, so with a JD in hand this step should cost **zero questions**.

Read Appendix: targeting for the archetype, level and company tables, and its five target questions when a candidate has given you nothing to work from.
...
Read Appendix: templates for the three structures, the five treatments, the rendering pipeline, and the formatting spec.
...
- [ ] **Every bullet scored against the five-check table in Appendix: bullets.** Report how many failed and on which check, name the worst two, and show each rewrite beside its original. Ticking this box without doing the scoring is how dead bullets ship
...
| Appendix: templates | Steps 5 and 6 | Two structures, five treatments, the visual preview, print CSS, PDF and DOCX routes |
