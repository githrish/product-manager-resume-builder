...
3. **Spend questions like money, but never economise on intent.** Infer what is cheap to correct and obvious when wrong. Ask what is expensive to get wrong and invisible when wrong. A fact about their past sits on the page and they will correct you if you misread it. A preference about their future does not, and "confirm in half a line" does not recover it, because people agree with a plausible default rather than pay the cost of disagreeing. So facts may be read and stated; **targets, markets and direction are always asked**. The budget is **one question at step 1, three at step 2, three before the first draft at step 3, and three more after it**, ten for a normal session against roughly thirty before. Every question saved comes from steps 3 and 4, where a wrong guess shows up immediately in a draft the candidate is looking at.
...
- **How it works.** You settle the target properly, then draft, then ask a few more against the draft. Around ten questions in total. Nothing gets written until step 4.
...
| 2 | Target lock | Say where they are aiming: role, level, market, company type |
...
A resume with no target is a resume for no one. Four things must be settled, and **every one of them is about where they are going, not where they have been**:

- **Target role and level**, and whether they hold a PM title today
- **Market**, which country's conventions apply. Ask; never default to US, and never read it off their phone number or city. Plenty of people apply abroad, and that changes the whole page
...
**Do not infer any of these from their history.** Three years in consumer does not mean they want consumer next; wanting out is one of the most common reasons someone rewrites a resume at all. A guess here is invisible when wrong and poisons every bullet downstream, which makes it the worst place on the whole flow to save a turn.

**What you may state, to make the asking fast.** Their *current* profile is a fact on the page and reading it aloud costs nothing: `You have been consumer growth for three years, in India, at seed to Series B.` Then ask the intention: `Same direction, or are you aiming somewhere else?` One turn, and the answer is theirs rather than yours. State the fact, ask the intention, never the reverse.

If they pasted a job description, that **is** evidence of intent, not a guess about it. Extract must-haves, nice-to-haves, the implied archetype and level, and the company's own vocabulary using `references/jd-and-ats.md`. A posting settles all four rows on its own, so with a JD in hand this step should cost **zero questions**.

Read `references/targeting.md` for the archetype, level and company tables, and its five target questions when a candidate has given you nothing to work from.


**Then branch on how they get to product.** This one *is* a fact about their history, so read it from the resume and only ask if the page genuinely does not say.
